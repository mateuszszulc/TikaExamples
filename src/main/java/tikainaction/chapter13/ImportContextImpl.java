...
/*
 * This is a copy of the ImportContextImpl class from Apache Jackrabbit.
 * The code has been slightly modified to fit the formatting of the
 * Tika in Action book.
 */
package tikainaction.chapter13;
...
public class ImportContextImpl implements ImportContext {
    ...
    public ImportContextImpl(
            Item importRoot, String systemId, InputContext ctx,
...
//<start id="webdav_type_detection"/>

Metadata metadata = new Metadata();
if (ctx != null && ctx.getContentType() != null) {
    metadata.set( //<co id="webdav_content_type"/>
            Metadata.CONTENT_TYPE, ctx.getContentType());
}
if (systemId != null) {
    metadata.set( //<co id="webdav_resource_name"/>
            Metadata.RESOURCE_NAME_KEY, systemId);
}
if (stream != null && !stream.markSupported()) {
    stream = new BufferedInputStream(stream);
}
type = detector.detect(stream, metadata); //<co id="webdav_detect"/>
//<end id="webdav_type_detection"/>
